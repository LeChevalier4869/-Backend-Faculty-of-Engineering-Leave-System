...
exports.getLeaveBalanceByUserId = async (req, res, next) => {
    ...
    try {
        if (!user || isNaN(userId)) throw createError(400, "รหัสผู้ใช้งานไม่ถูกต้อง");

        const leaveBalance = await LeaveBalanceService.getLeaveBalanceByUserId(userId);
...
exports.getLeaveBalanceMe = async (req, res, next) => {
    try {
        const userId = req.user.id;
        const balances = await LeaveBalanceService.getLeaveBalanceByUserId(userId);
